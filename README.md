...
I would not begin by building a very large crawler.

I would test additional sources on a new benchmark in this order:

1. customer invoice data;
2. EORI and other adjacent identifiers;
3. better company-to-domain resolution;
4. large-scale website and document crawling;
5. automated HMRC verification;
6. manual review for ambiguous entity relationships.

After each stage, I would measure the additional verified relationships, errors and cost. A source that added little coverage or produced many wrong-entity candidates would be stopped.

## 2. Proposed Production Pipeline

The production process should keep discovery, verification and attribution separate.

### Company resolution

Supplier records would first be matched to Companies House entities using:
...
Domain candidates could come from:

-supplier email addresses;
-procurement records;
-search APIs;
-commercial company-domain datasets;
-customer records;
-public company documents.

A domain would need evidence such as a company number, legal name, address or explicit trading-name relationship before being accepted.

### Crawling and extraction

For confirmed domains, I would crawl:

-homepage and footer;
-Terms and Legal pages;
-Privacy, Contact and About pages;
-payment, delivery and returns pages;
-invoices and supplier documents;
-relevant PDFs.

The extractor would focus on numbers near labels such as "VAT number", "VAT registration", "VAT No" and "VAT Reg".

For each candidate, the system would retain the source URL, original text, context, retrieval date, target company number and source-content hash.

### Verification and attribution

Every candidate would be checked through the HMRC API.

The HMRC name and address would then be compared with the target company. Group, parent-company and trading-name cases would be sent to review.
